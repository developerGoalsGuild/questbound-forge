...
# AppSync stack deployment script
...
AUTO_APPROVE="true"
SKIP_INIT=""
TF_LOG_PATH="$HOME/terraform-logs/tf-appsync.log"
...
STACK_PATH="$TERRAFORM_DIR/stacks/appsync"
...
print_info "Starting AppSync stack deployment for environment: $ENV"
...
(
    ...
    if [ $? -eq 0 ]; then
        print_success "AppSync stack deployment completed"
    else
        print_error "AppSync stack deployment failed"
        exit 1
    fi
)
