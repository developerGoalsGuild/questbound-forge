...
# API Gateway stack deployment script
...
AUTO_APPROVE="true"
SKIP_INIT=""
TF_LOG_PATH="$HOME/terraform-logs/tf-apigateway.log"
...
STACK_PATH="$TERRAFORM_DIR/stacks/apigateway"
...
print_info "Starting API Gateway stack deployment for environment: $ENV"
...
(
    ...
    if [ $? -eq 0 ]; then
        print_success "API Gateway stack deployment completed"
    else
        print_error "API Gateway stack deployment failed"
        exit 1
    fi
)
