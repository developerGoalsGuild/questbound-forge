...
# This script builds Docker image, auto-increments version, and deploys gamification-service
param(
...
$LogFile = "$LogDir\tf-gamification-service.log"
...
function Write-Log {
  ...
  $LogEntry = "[$Timestamp] [$Level] [gamification-service-build-deploy] $Message"
...
function Build-AndPush-Image {
  ...
  Write-Log "Building Docker image for gamification-service..." "INFO"
  ...
  Write-Log "Building version $ImageTag for gamification-service" "INFO"
  ...
  try {
    ...
    $DockerfilePath = "services\gamification-service\Dockerfile"
    ...
    try {
      ...
      if ($process.ExitCode -ne 0) {
        ...
        throw "Docker build failed for gamification-service"
      }
    ...
    try {
      ...
      if ($pushProcess.ExitCode -ne 0) {
        ...
        throw "Docker push failed for gamification-service"
      }
...
function Deploy-Service {
  ...
  Write-Log "Deploying gamification-service with image: $ImageUri" "INFO"
  ...
  try {
    if (-not $SkipInit) {
      Write-Log "Running terraform init for gamification-service" "INFO"
      terraform -chdir="$StackPath" init -upgrade
      
      if ($LASTEXITCODE -ne 0) {
        throw "Terraform init failed for gamification-service"
      }
    }
    
    if ($PlanOnly) {
      Write-Log "Running terraform plan for gamification-service" "INFO"
      terraform -chdir="$StackPath" plan -var-file "$EnvFile"
    } else {
      Write-Log "Running terraform apply for gamification-service" "INFO"
      if ($AutoApprove) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Terraform deployment failed for gamification-service"
    }
    
    Write-Log "Gamification-service deployment completed successfully" "INFO"
  } finally {
    Pop-Location
  }
}
...
# Main execution
try {
  Write-Log "Starting gamification-service build and deployment for environment: $Env" "INFO"
  ...
  $ServicePath = "$RepoRoot\..\..\services\gamification-service"
  $StackPath = "$RepoRoot\stacks\services\gamification-service"
  $EnvFile = Resolve-Path "$RepoRoot\environments\$Env.tfvars" | Select-Object -ExpandProperty Path
  
  # Build and push Docker image
  $ImageUri = Build-AndPush-Image -ServicePath $ServicePath -ECRRepository "goalsguild_gamification_service" -AccountId $AWSInfo.AccountId -Region $AWSInfo.Region
  
  # Deploy service
  Deploy-Service -ImageUri $ImageUri -StackPath $StackPath -EnvFile $EnvFile -RepoRoot $RepoRoot
  
  Write-Log "Gamification-service build and deployment completed successfully!" "INFO"
  Write-Host "`nGamification-Service Deployment Summary:" -ForegroundColor Green
  ...
  $ErrorMessage = "Error in gamification-service deployment: $($_.Exception.Message)"
  ...
  Write-Host "`nGamification-service deployment failed!" -ForegroundColor Red
  throw
}
