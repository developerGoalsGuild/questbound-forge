...
$LogFile = "$LogDir\tf-user-service.log"
...
function Write-Log {
  ...
  $LogEntry = "[$Timestamp] [$Level] [user-service-build-deploy] $Message"
...
function Build-AndPush-Image {
  ...
  Write-Log "Building Docker image for user-service..." "INFO"
  ...
  Write-Log "Building version $ImageTag for user-service" "INFO"
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Docker build failed for user-service"
    }
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Docker push failed for user-service"
    }
...
function Deploy-Service {
  ...
  Write-Log "Deploying user-service with image: $ImageUri" "INFO"
  ...
  try {
    if (-not $SkipInit) {
      Write-Log "Running terraform init for user-service" "INFO"
      terraform -chdir="$StackPath" init -upgrade
      
      if ($LASTEXITCODE -ne 0) {
        throw "Terraform init failed for user-service"
      }
    }
    
    if ($PlanOnly) {
      Write-Log "Running terraform plan for user-service" "INFO"
      terraform -chdir="$StackPath" plan -var-file "$EnvFile"
    } else {
      Write-Log "Running terraform apply for user-service" "INFO"
      if ($AutoApprove) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Terraform deployment failed for user-service"
    }
...
# Main execution
try {
  Write-Log "Starting user-service build and deployment for environment: $Env" "INFO"
  ...
  $ServicePath = "$RepoRoot\..\..\services\user-service"
  ...
  # Build and push Docker image
  $ImageUri = Build-AndPush-Image -ServicePath $ServicePath -ECRRepository "goalsguild_user_service" -AccountId $AWSInfo.AccountId -Region $AWSInfo.Region
  
  # Deploy service
  Deploy-Service -ImageUri $ImageUri -StackPath $StackPath -EnvFile $EnvFile -RepoRoot $RepoRoot
  
  Write-Log "User-service build and deployment completed successfully!" "INFO"
  Write-Host "`nUser-Service Deployment Summary:" -ForegroundColor Green
  ...
  $ErrorMessage = "Error in user-service deployment: $($_.Exception.Message)"
  ...
  Write-Host "`nUser-service deployment failed!" -ForegroundColor Red
  throw
}
