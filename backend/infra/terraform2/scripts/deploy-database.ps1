...
function Write-Log {
  ...
  $LogEntry = "[$Timestamp] [$Level] [database] $Message"
  Write-Host $LogEntry
  Add-Content -Path $LogFile -Value $LogEntry
}
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot = Resolve-Path "$Root\.." | Select-Object -ExpandProperty Path
$StackPath = Resolve-Path "$RepoRoot\stacks\database" | Select-Object -ExpandProperty Path
$EnvFile = Resolve-Path "$RepoRoot\environments\$Env.tfvars" | Select-Object -ExpandProperty Path
Write-Log "Starting database stack deployment for environment: $Env" "INFO"
...
try {
  ...
  Write-Log "Database stack deployment completed" "INFO"
} catch {
  Write-Log "Error in database stack: $($_.Exception.Message)" "ERROR"
  throw
} finally { 
  Pop-Location 
}
