...
# This script builds Docker image, auto-increments version, and deploys collaboration-service
param(
...
$LogFile = "$LogDir\tf-collaboration-service.log"
...
function Write-Log {
  ...
  $LogEntry = "[$Timestamp] [$Level] [collaboration-service-build-deploy] $Message"
...
function Build-AndPush-Image {
  ...
  Write-Log "Building Docker image for collaboration-service..." "INFO"
  ...
  Write-Log "Building version $ImageTag for collaboration-service" "INFO"
  ...
  try {
    ...
    $DockerfilePath = "services\collaboration-service\Dockerfile"
    ...
    try {
      ...
      if ($process.ExitCode -ne 0) {
        ...
        throw "Docker build failed for collaboration-service"
      }
    ...
    try {
      ...
      if ($pushProcess.ExitCode -ne 0) {
        ...
        throw "Docker push failed for collaboration-service"
      }
...
function Deploy-Service {
  ...
  Write-Log "Deploying collaboration-service with image: $ImageUri" "INFO"
  ...
  try {
    if (-not $SkipInit) {
      Write-Log "Running terraform init for collaboration-service" "INFO"
      terraform -chdir="$StackPath" init -upgrade
      
      if ($LASTEXITCODE -ne 0) {
        throw "Terraform init failed for collaboration-service"
      }
    }
    
    if ($PlanOnly) {
      Write-Log "Running terraform plan for collaboration-service" "INFO"
      terraform -chdir="$StackPath" plan -var-file "$EnvFile"
    } else {
      Write-Log "Running terraform apply for collaboration-service" "INFO"
      if ($AutoApprove) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Terraform deployment failed for collaboration-service"
    }
    
    Write-Log "collaboration-service deployment completed successfully" "INFO"
  } finally {
    Pop-Location
  }
}
...
# Main execution
try {
  Write-Log "Starting collaboration-service build and deployment for environment: $Env" "INFO"
  ...
  $ServicePath = "$RepoRoot\..\..\services\collaboration-service"
  $StackPath = "$RepoRoot\stacks\services\collaboration-service"
  $EnvFile = Resolve-Path "$RepoRoot\environments\$Env.tfvars" | Select-Object -ExpandProperty Path
  
  # Build and push Docker image
  $ImageUri = Build-AndPush-Image -ServicePath $ServicePath -ECRRepository "goalsguild_collaboration_service" -AccountId $AWSInfo.AccountId -Region $AWSInfo.Region
  
  # Deploy service
  Deploy-Service -ImageUri $ImageUri -StackPath $StackPath -EnvFile $EnvFile -RepoRoot $RepoRoot
  
  Write-Log "collaboration-service build and deployment completed successfully!" "INFO"
  Write-Host "`ncollaboration-service Deployment Summary:" -ForegroundColor Green
  ...
  $ErrorMessage = "Error in collaboration-service deployment: $($_.Exception.Message)"
  ...
  Write-Host "`ncollaboration-service deployment failed!" -ForegroundColor Red
  throw
}
