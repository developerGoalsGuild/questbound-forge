...
function Write-Log {
  ...
  $LogEntry = "[$Timestamp] [$Level] [appsync] $Message"
  Write-Host $LogEntry
  Add-Content -Path $LogFile -Value $LogEntry
}
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot = Resolve-Path "$Root\.." | Select-Object -ExpandProperty Path
$StackPath = Resolve-Path "$RepoRoot\stacks\appsync" | Select-Object -ExpandProperty Path
$EnvFile = Resolve-Path "$RepoRoot\environments\$Env.tfvars" | Select-Object -ExpandProperty Path
Write-Log "Starting appsync stack deployment for environment: $Env" "INFO"
...
try {
  ...
  Write-Log "Appsync stack deployment completed" "INFO"
} catch {
  Write-Log "Error in appsync stack: $($_.Exception.Message)" "ERROR"
  throw
} finally { 
  Pop-Location 
}
