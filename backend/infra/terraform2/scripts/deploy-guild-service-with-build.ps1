...
# This script builds Docker image, auto-increments version, and deploys guild-service
param(
  [ValidateSet("dev","staging","prod")] [string]$Env = "dev",
  [switch]$PlanOnly,
  [switch]$AutoApprove = $true,
  [switch]$SkipInit,
  [string]$TfLogPath = 'D:\terraformLogs\tf-guild-service.log'
)
...
$LogFile = "$LogDir\tf-guild-service.log"
...
function Write-Log {
  ...
  $LogEntry = "[$Timestamp] [$Level] [guild-service-build-deploy] $Message"
...
function Build-AndPush-Image {
  ...
  Write-Log "Building Docker image for guild-service..." "INFO"
  ...
  Write-Log "Building version $ImageTag for guild-service" "INFO"
  ...
  try {
    ...
    $DockerfilePath = "services\guild-service\Dockerfile"
    ...
    try {
      ...
      if ($process.ExitCode -ne 0) {
        ...
        throw "Docker build failed for guild-service"
      }
    ...
    try {
      ...
      if ($pushProcess.ExitCode -ne 0) {
        ...
        throw "Docker push failed for guild-service"
      }
...
function Deploy-Service {
  ...
  Write-Log "Deploying guild-service with image: $ImageUri" "INFO"
  ...
  try {
    if (-not $SkipInit) {
      Write-Log "Running terraform init for guild-service" "INFO"
      terraform -chdir="$StackPath" init -upgrade
      
      if ($LASTEXITCODE -ne 0) {
        throw "Terraform init failed for guild-service"
      }
    }
    
    if ($PlanOnly) {
      Write-Log "Running terraform plan for guild-service" "INFO"
      terraform -chdir="$StackPath" plan -var-file "$EnvFile"
    } else {
      Write-Log "Running terraform apply for guild-service" "INFO"
      if ($AutoApprove) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Terraform deployment failed for guild-service"
    }
    
    Write-Log "Guild-service deployment completed successfully" "INFO"
  } finally {
    Pop-Location
  }
}
...
# Main execution
try {
  Write-Log "Starting guild-service build and deployment for environment: $Env" "INFO"
  ...
  $ServicePath = "$RepoRoot\..\..\services\guild-service"
  $StackPath = "$RepoRoot\stacks\services\guild-service"
  $EnvFile = Resolve-Path "$RepoRoot\environments\$Env.tfvars" | Select-Object -ExpandProperty Path
  
  # Build and push Docker image
  $ImageUri = Build-AndPush-Image -ServicePath $ServicePath -ECRRepository "goalsguild_guild_service" -AccountId $AWSInfo.AccountId -Region $AWSInfo.Region
  
  # Deploy service
  Deploy-Service -ImageUri $ImageUri -StackPath $StackPath -EnvFile $EnvFile -RepoRoot $RepoRoot
  
  Write-Log "Guild-service build and deployment completed successfully!" "INFO"
  Write-Host "`nGuild-Service Deployment Summary:" -ForegroundColor Green
  ...
  $ErrorMessage = "Error in guild-service deployment: $($_.Exception.Message)"
  ...
  Write-Host "`nGuild-service deployment failed!" -ForegroundColor Red
  throw
}
