...
AUTO_APPROVE="true"
SKIP_INIT=""
TF_LOG_PATH="$HOME/terraform-logs/tf-database.log"
...
STACK_PATH="$TERRAFORM_DIR/stacks/database"
...
print_info "Starting database stack deployment for environment: $ENV"
...
(
    ...
    if [ $? -eq 0 ]; then
        print_success "Database stack deployment completed"
    else
