...
AUTO_APPROVE="true"
SKIP_INIT=""
TF_LOG_PATH="$HOME/terraform-logs/tf-ses.log"
...
STACK_PATH="$TERRAFORM_DIR/stacks/ses"
...
print_info "Starting SES stack deployment for environment: $ENV"
...
(
    ...
    if [ $? -eq 0 ]; then
        print_success "SES stack deployment completed"
    else
        print_error "SES stack deployment failed"
        exit 1
    fi
)
