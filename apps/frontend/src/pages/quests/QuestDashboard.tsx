...
const QuestDashboard: React.FC = () => {
  ...
  // Debounce refresh calls to reduce API calls when multiple actions happen quickly
  ...
  const debouncedRefresh = useCallback(() => {
    ...
    }, 500); // Wait 500ms before refreshing to batch multiple rapid actions
  }, [refresh]);
  ...
  const handleManualQuestCheck = async () => {
    ...
    try {
      ...
      // Refresh quests if any were completed (use debounced refresh to reduce API calls)
  ...
  const handleStart = async (id: string) => {
    try {
      ...
      // Use debounced refresh to reduce API calls when multiple actions happen quickly
      debouncedRefresh();
    } catch (error) {
              ...
              <CardContent>
                ...
                >
                  <Suspense fallback={
                    ...
                    {/* Pass isExpanded prop to control auto-refresh */}
                    <QuestAnalyticsDashboard isExpanded={isAnalyticsExpanded} />
                  </Suspense>
                </ErrorBoundary>
              </CardContent>
            )}
