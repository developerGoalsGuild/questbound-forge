...
export function RoomSettingsDialog({
  ...
  const chatT = (t as any)?.chat?.settings;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: chatT?.settingsSaved || "Settings saved",
        description: chatT?.settingsSavedDesc || "Room settings have been updated successfully.",
        variant: "default",
      });
      ...
      toast({
        title: chatT?.errorSaving || "Error saving settings",
      ...
      <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {chatT?.title || 'Room Settings'}
          </DialogTitle>
          <DialogDescription>
            {chatT?.configureFor || 'Configure settings for'} {roomName || roomId}
            {roomType === 'guild' && (
              <Badge variant="secondary" className="ml-2">
                <Shield className="h-3 w-3 mr-1" />
                {chatT?.guildRoom || 'Guild Room'}
              </Badge>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-4">
          ...
          {isLoading ? (
            ...
            <>
              <div className="space-y-2">
                <Label htmlFor="room-name">{chatT?.roomName || 'Room Name'}</Label>
                <Input
                  id="room-name"
                  value={settings.name}
                  onChange={(e) => handleFieldChange('name', e.target.value)}
                  placeholder={chatT?.roomNamePlaceholder || 'Enter room name'}
                  disabled={roomType === 'guild'}
                />
                {roomType === 'guild' && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {chatT?.guildRoomNote || 'Guild room names are managed by guild settings'}
                  </p>
                )}
              </div>
              ...
              {roomType === 'general' && (
                <div className="space-y-2">
                  <Label htmlFor="room-description">{chatT?.description || 'Description'}</Label>
                  <Textarea
                    id="room-description"
                    value={settings.description || ''}
                    onChange={(e) => handleFieldChange('description', e.target.value)}
                    placeholder={chatT?.descriptionPlaceholder || 'Enter room description'}
                    rows={3}
                  />
                </div>
              )}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="room-public">{chatT?.publicRoom || 'Public Room'}</Label>
                  <Switch
                    id="room-public"
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {settings.isPublic 
                    ? (chatT?.publicRoomDesc || 'Anyone can join this room')
                    : (chatT?.privateRoomDesc || 'Only invited members can join this room')}
                </p>
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="room-reactions">{chatT?.allowReactions || 'Allow Reactions'}</Label>
                  <Switch
                    id="room-reactions"
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {chatT?.allowReactionsDesc || 'Allow users to react to messages with emojis'}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="max-length">{chatT?.maxMessageLength || 'Max Message Length'}</Label>
                <Input
                  id="max-length"
                  type="number"
                ...
                />
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {chatT?.maxMessageLengthDesc || 'Maximum characters allowed per message (100 - 10,000)'}
                </p>
              </div>
              ...
              {roomInfo && 'memberCount' in roomInfo && (
                <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-400">{chatT?.activeMembers || 'Active Members'}</span>
        ...
        <DialogFooter>
          ...
          >
            {isSaving ? (
              ...
              chatT?.saveChanges || 'Save Changes'
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
