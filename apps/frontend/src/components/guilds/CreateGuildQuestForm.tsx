/**
 * CreateGuildQuestForm Component
 *
 * Form for creating new guild quests (quantitative or percentual only).
 */
...
import { guildAPI, GuildQuestCreateInput } from '@/lib/api/guild';
import { toast } from 'sonner';
import { getGuildTranslations } from '@/i18n/guild';
import { useTranslation } from '@/hooks/useTranslation';

interface CreateGuildQuestFormProps {
  guildId: string;
  onSuccess?: () => void;
  onCancel?: () => void;
}
...
export const CreateGuildQuestForm: React.FC<CreateGuildQuestFormProps> = ({
  guildId,
  onSuccess,
  onCancel,
}) => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: GuildQuestCreateInput) => guildAPI.createGuildQuest(guildId, data),
    onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate(formData as GuildQuestCreateInput);
  };
