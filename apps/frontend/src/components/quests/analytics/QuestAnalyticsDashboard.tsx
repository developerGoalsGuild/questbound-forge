...
interface QuestAnalyticsDashboardProps {
  className?: string;
  isExpanded?: boolean; // Control auto-refresh based on expansion state
}

export const QuestAnalyticsDashboard: React.FC<QuestAnalyticsDashboardProps> = ({ className, isExpanded = true }) => {
  ...
  const {
    ...
    autoRefresh: isExpanded, // Only auto-refresh when expanded (reduces API calls when collapsed)
    refreshInterval: 300000, // Use optimized default (5 minutes) - reduced from 60s to reduce API calls by 83%
    enabled: isExpanded // Only load data when expanded
  });
